...
const NewTemplateScreen = ({ navigation, route }) => {
  ...
  const getSupportedLanguages = async () => {
    ...
    console.log("allSuportedLanguage", allSupportedLanguagesdata.data);
  ...
  const handleAddBodyVariable = () => {
    ...
    console.log("arrayBody", JSON.stringify(array));
  };
  ...
  const populateButtons = async (item) => {
    ...
    console.log("localArray.length", localArray.length);
    console.log("localArray.length", localArray);
    for (let s = 0; s < localArray.length; s++) {
      if (localArray[s].type == "QUICK_REPLY") {
        console.log("2");
        // console.log('setSelectButtonType',selectedButtonType)
  ...
  const getVariableData = async () => {
    ...
    console.log(
      "responseVariableData",
      JSON.stringify(variableDataResponse.data)
    );
  };
  ...
  const updateTemplate = async () => {
    console.log("setBodyVariableData", bodyVariableData);
    console.log("selectedButtonVariable");
    return false;
    if (data.length > 0) {
      setPhoneButtonText();
    ...
    if (
      ...
      console.log("1", phoneButtonText?.length, visitWebSiteButtonText?.length);
      component.push({
        type: "BUTTONS",
        buttons: [
          {
    ...
    if (
      ...
      console.log("2", phoneButtonText?.length, visitWebSiteButtonText?.length);
      component.push({
        type: "BUTTONS",
        buttons: [
          {
    ...
    if (phoneButtonText?.length > 0 && visitWebSiteButtonText?.length > 0) {
      console.log("3", phoneButtonText?.length, visitWebSiteButtonText?.length);
      component.push({
        type: "BUTTONS",
        buttons: [
          {
    ...
    console.log("addPayload", component);
    return false;
    ...
    console.log("Update Template Reponse", updateTemplate);
  };
  ...
  const createTemplate = async () => {
    ...
    // console.log('callToActionButtonName',callToActionButtonName)
    // return false
    for (let i = 0; i < callToActionButtonName.length; i++) {
      if (callToActionButtonName[i]?.name == "Call Phone") {
        if (
  ...
  const handleKeyPress = ({ nativeEvent }) => {
    ...
    if (nativeEvent.key === "Backspace") {
      ...
      console.log("testName", mediaTextName);
  ...
  const onChangeMediaName = (val) => {
    ...
    if (!variable) {
      if (val?.includes(`{{1}}`)) {
        setVariable(true);
        console.log(`Value found: {{${variableCount}}}`);
      }
    }
    // if(variable){
