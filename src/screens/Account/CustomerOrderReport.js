...
const CustomerOrderReport = ({navigation, title}) => {
  ...
  const salesReports = async (pageNumber = 1) => {
    ...
    setTotalSales(prevSales =>
      pageNumber === 1
        ? reportsResponse.data.data.customer_order
        : [...prevSales, ...reportsResponse.data.data.customer_order],
    );
