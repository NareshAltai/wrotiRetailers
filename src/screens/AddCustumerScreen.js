...
import {View,Text,StatusBar,Image,TouchableOpacity,TextInput,StyleSheet,ScrollView,CheckBox,FlatList} from "react-native";
...
const AddCustumerScreen = ({ navigation, route }) => {
  ...
  const getGroupsByCustomer = async () => {
    ...
    if (customerResponse.data.success == true) {
      setSelectedGroupId(
        customerResponse.data.customeringroups.map(
          (obj) => obj.customer_group_id
        )
      );
  ...
  useEffect(() => {
    setTimeout(async () => {
      ...
      if (
        route != undefined &&
        route.params?.customerObject?.isEdited == true
      ) {
        setIsEdited(true);
      } else {
        setIsEdited(false);
